...
import { AutherSchema } from '../../../lib/validaters/autherDataValidator'
...
export async function POST(request) {
    try {
        ...
        if (autherAlreadyExists) return NextResponse.json({ message: "You Have Already A Profile", status: 'error' }, { status: 401 })
        ...
        if (!isNormalUser) return NextResponse.json({ message: "Not Allowed", status: 'error' }, { status: 401 })
        ...
        const { infoData, educationData, experienceData, socialLinks, skills } = await request.json();
        ...
        if (!addAuther) return NextResponse.json({ message: "Internal Server Error", status: 'error' }, { status: 401 })

        return NextResponse.json(
            { message: "Your Application has been submitted", status: 'success', auther: addAuther },
            { status: 200 }
        )
    } catch (error) {
