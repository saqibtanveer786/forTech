...
export async function updateAuthor(data, id) {
  try {
    ...
    console.log("updated author is: ", updateAuthor);
...
export async function getAuthorsData(id) {
  try {
    const url = `${process.env.HOST}/api/authorsdata?userid=${id}`;
    const response = await fetch(url, {
      method: "POST",
      cache: "no-cache",
      headers: {
...
// change user image
export async function changeUserData(id, image, name, email) {
  try {
    let data = {};
