...
export default function AutherRegistrationForm({ userId }) {
    ...
    const [infoData, setInfoData] = useState({ id: userId, birthDate: "2023-10-12T05:51" })
    ...
    const { showAlert,
        ...
        setAlertMessage,
        alertStatus,
        setAlertStatus } = useContext(AlertContext)
                                ...
                                <div className="mb-5.5">
                                    <ul className="py-2">
                                        {skills && skills.map((item, i) => {
                                            return <li key={i} id={i} className="relative border px-2 py-3 my-2 ">{item}<span className="absolute right-4 top-[7px]"><button
                                                className="text-red-600 py-1 px-2"
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    const updatedLinks = skills.filter((li) => {
                                                        const valueToDelete = document.getElementById(i).innerHTML.split("<")[0];
                                                        console.log("valueToDelete", valueToDelete, li)
                                    ...
                                    /><button
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            const valueToAppend = document.getElementById("skills").value
