  ...
  return (
    <>
      <div className="absolute bottom-0 right-0 group flex items-center justify-center p-6 h-20">
        ...
        <Link
          href={`/updateblog/${blogId}`}
          className="absolute bottom-0 right-1 rounded-full transition-all duration-[0.2s] ease-out scale-x-0 group-hover:scale-x-100 group-hover:-translate-y-14 flex p-2 hover:p-3 bg-green-300 hover:bg-green-400 text-white"
        >
          <RxUpdate size={21} className="cursor-pointer" />
        </Link>

        {/* Toggler Button */}
        <button className="flex items-center justify-center text-white bg-primary rounded-full w-12 h-12 hover:bg-indigo-500 focus:ring-blue-300 absolute bottom-0 right-0 ">
          <AiOutlinePlus
            id={blogId}
